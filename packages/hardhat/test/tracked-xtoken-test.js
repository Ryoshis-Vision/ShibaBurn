...
describe('Shiba Burn', function(accounts) {
  ...
  var trackingThreshold = 1000000n;
  ...
  it("should burn shib from our frens", async function() {
    ...
    expect(burntShibBalance).to.equal(shibBalance.toString());
  ...
  it("allows frens to burn multiple times", async function() {
    ...
    expect(await shibaburn.totalTrackedBurners(shib.address, 0)).to.eq(1);
    ...
    console.log("expect fail, unapproved::");
    var tx = await expectBurn("SafeMath: subtraction overflow",
      trackingThreshold-1n, accounts[1],
    );
    ...
    expect(await shibaburn.amountBurnt(shib.address, 0, accounts[1])).to.eq(trackingThreshold-1n);
    expect(await shibaburn.totalBurnt(shib.address, 0)).to.eq(previouslyBurnt + trackingThreshold-1n);
    expect(await shibaburn.totalTrackedBurners(shib.address, 0)).to.eq(1);

    var tx = await expectBurn("success",
      shibBalance - (trackingThreshold-1n), accounts[1],
    );
    ...
    expect(await shibaburn.totalTrackedBurners(shib.address, 0)).to.eq(2);
  });
  ...
  it("allows frens to buy and burn in one TX", async function() {
    ...
    expect(await shibaburn.totalTrackedBurners(shib.address, 0)).to.eq(3);
